...
public class primerSecondStageProjectile : MonoBehaviour {
	...
	void Start () {
		storedCenterOfMass = spawnOrigin.GetComponent<primerInternalEmitter> ().centerOfMass; //gets the center of mass from the reference to its originator upon creation, otherwise it would be a non-satic value
	}
